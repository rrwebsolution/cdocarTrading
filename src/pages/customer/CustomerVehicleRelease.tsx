import { useCallback, useEffect, useMemo, useState } from "react"
import { KeyRound, LoaderCircle, RefreshCw } from "lucide-react"
import { toast } from "react-toastify"

import { Button } from "@/components/ui/button"
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card"
import { getApiList } from "@/lib/operations"
import { cn } from "@/lib/utils"
import { getCurrentCustomer } from "./customerApi"

type VehicleReleaseRecord = {
  checklist_status?: string
  customer?: {
    id?: number
    name?: string
  }
  customer_id?: number
  document_status?: string
  id?: number
  reference?: string
  released_at?: string
  released_by?: string
  remarks?: string
  status?: string
  vehicle?: {
    name?: string
    plate_number?: string
  }
}

function CustomerVehicleRelease() {
  const [releases, setReleases] = useState<VehicleReleaseRecord[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [isRefreshing, setIsRefreshing] = useState(false)

  const loadReleases = useCallback(async (forceRefresh = false) => {
    const [customer, data] = await Promise.all([
      getCurrentCustomer(forceRefresh),
      getApiList<VehicleReleaseRecord>("/api/vehicle-releases", forceRefresh),
    ])
    const customerId = customer?.id ? String(customer.id) : ""

    setReleases(
      data.filter((release) =>
        String(release.customer_id ?? release.customer?.id ?? "") === customerId,
      ),
    )
  }, [])

  useEffect(() => {
    loadReleases()
      .catch(() => toast.error("Unable to load vehicle release status."))
      .finally(() => setIsLoading(false))
  }, [loadReleases])

  const stats = useMemo(() => {
    const ready = releases.filter((release) =>
      ["ready", "ready for release"].includes((release.status ?? "").toLowerCase()),
    ).length
    const released = releases.filter((release) =>
      (release.status ?? "").toLowerCase() === "released",
    ).length

    return [
      ["Release Records", releases.length],
      ["Ready", ready],
      ["Released", released],
    ]
  }, [releases])

  const refreshData = async () => {
    setIsRefreshing(true)

    try {
      await loadReleases(true)
    } catch {
      toast.error("Unable to refresh vehicle release status.")
    } finally {
      setIsRefreshing(false)
    }
  }

  const isBusy = isLoading || isRefreshing

  return (
    <div className="grid gap-4">
      <Card>
        <CardHeader className="flex-row items-start justify-between gap-4 max-lg:flex-col">
          <div className="flex min-w-0 items-start gap-4 max-sm:flex-col">
            <span className="grid size-12 shrink-0 place-items-center rounded-lg bg-primary text-primary-foreground">
              <KeyRound aria-hidden="true" className="size-5" />
            </span>
            <div>
              <p className="mb-3 text-xs font-extrabold uppercase tracking-[0.13em] text-primary">
                Customer Portal Module
              </p>
              <CardTitle className="text-3xl font-black tracking-normal max-sm:text-2xl">
                Vehicle Release Status
              </CardTitle>
              <CardDescription className="mt-3 max-w-3xl leading-7">
                View release readiness, document verification, checklist status,
                and turnover details for your vehicle.
              </CardDescription>
            </div>
          </div>
          <Button disabled={isBusy} onClick={() => void refreshData()} variant="outline">
            <RefreshCw aria-hidden="true" className={cn("size-4", isBusy && "animate-spin")} />
            {isBusy ? "Refreshing..." : "Refresh Data"}
          </Button>
        </CardHeader>
      </Card>

      <div className="grid grid-cols-3 gap-4 max-lg:grid-cols-1">
        {stats.map(([label, value]) => (
          <Card key={label}>
            <CardContent className="flex items-center gap-4 p-4">
              <span className="grid size-11 shrink-0 place-items-center rounded-lg bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-200">
                <KeyRound aria-hidden="true" className="size-5" />
              </span>
              <div>
                <p className="text-sm font-bold text-muted-foreground">{label}</p>
                <strong className="mt-2 block text-3xl leading-none">{value}</strong>
              </div>
            </CardContent>
          </Card>
        ))}
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Release Status</CardTitle>
          <CardDescription>
            Latest release and turnover status from the backend.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-3">
          {isLoading ? (
            <div className="grid min-h-40 place-items-center rounded-lg border border-dashed border-border bg-muted/40 p-6">
              <LoaderCircle aria-hidden="true" className="size-6 animate-spin text-primary" />
            </div>
          ) : releases.length === 0 ? (
            <div className="rounded-lg border border-dashed border-border bg-muted/40 p-6 text-center">
              <p className="font-black">No release records yet</p>
              <p className="mt-1 text-sm font-medium text-muted-foreground">
                Vehicle release status will appear once prepared by staff.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-3 max-xl:grid-cols-1">
              {releases.map((release, index) => (
                <div className="rounded-lg border border-border p-4" key={release.id ?? index}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="truncate font-black">{release.reference ?? "Release Record"}</p>
                      <p className="mt-1 text-sm font-semibold text-muted-foreground">
                        {release.vehicle?.name ?? "Vehicle not specified"}
                      </p>
                    </div>
                    <span className={cn("rounded-full border px-3 py-1 text-xs font-black", statusClass(release.status))}>
                      {titleCase(release.status ?? "Pending")}
                    </span>
                  </div>
                  <div className="mt-4 grid gap-2 text-sm text-muted-foreground">
                    <Info label="Plate Number" value={release.vehicle?.plate_number ?? "N/A"} />
                    <Info label="Checklist" value={titleCase(release.checklist_status ?? "Pending")} />
                    <Info label="Documents" value={titleCase(release.document_status ?? "Pending")} />
                    <Info label="Released Date" value={formatDate(release.released_at)} />
                    <Info label="Released By" value={release.released_by ?? "N/A"} />
                    <Info label="Remarks" value={release.remarks ?? "N/A"} />
                  </div>
                </div>
              ))}
            </div>
          )}
        </CardContent>
      </Card>
    </div>
  )
}

function Info({ label, value }: { label: string; value: string }) {
  return (
    <p>
      <span className="font-bold text-foreground">{label}:</span> {value}
    </p>
  )
}

function statusClass(value?: string) {
  const status = value?.toLowerCase() ?? "pending"

  if (["released", "ready", "ready for release"].includes(status)) {
    return "border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900/60 dark:bg-emerald-950/30 dark:text-emerald-200"
  }

  if (["processing", "for review", "pending"].includes(status)) {
    return "border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-200"
  }

  if (["cancelled", "rejected", "blocked"].includes(status)) {
    return "border-rose-200 bg-rose-50 text-rose-700 dark:border-rose-900/60 dark:bg-rose-950/30 dark:text-rose-200"
  }

  return "border-blue-200 bg-blue-50 text-blue-700 dark:border-blue-900/60 dark:bg-blue-950/30 dark:text-blue-200"
}

function titleCase(value: string) {
  return value
    .split(/[\s_-]+/)
    .filter(Boolean)
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
    .join(" ")
}

function formatDate(value?: string) {
  if (!value) {
    return "N/A"
  }

  const date = new Date(value)

  if (Number.isNaN(date.getTime())) {
    return value
  }

  return new Intl.DateTimeFormat("en-PH", { dateStyle: "medium" }).format(date)
}

export default CustomerVehicleRelease
